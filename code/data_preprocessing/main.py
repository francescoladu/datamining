...
def main() -> None:
    ...
    for dataset_name in config.DATASET_PATHS:
        ...
        try:
            data, X, y, sample_weight = data_processing.load_and_prepare_dataset(
                dataset_name
            )
        except Exception as error:
            print(f"Execution error loading dataset '{dataset_name}': {error}")
            print("Verify that clean_and_split.py was run successfully.\n")
            continue

        print(
            f"Dataset size: {X.shape[0]} unique profiles with {X.shape[1]} features "
            f"(Total weighted instances: {sample_weight.sum():.0f})."
        )
        ...
        statistics_path = config.OUTPUT_DIR / f"dataset_statistics_{dataset_name}.csv"
        ...
        conflict_statistics_path = (
            config.OUTPUT_DIR / f"conflicting_profiles_statistics_{dataset_name}.csv"
        )
        ...
        if not conflicting_profiles.empty:
            ...
            print(f"-> Saved list of conflicting profiles: {conflicting_profiles_path.name}")

        # 3. Mutual Information (calculated on unique predictor space)
        print("\n[3/6] Ranking features with Mutual Information...")
        mutual_information = data_processing.calculate_mutual_information(X, y)
        mutual_information_path = config.OUTPUT_DIR / f"mutual_information_{dataset_name}.csv"
        ...
        correlation_matrix = data_processing.calculate_spearman_correlation_matrix(relevant_X)
        ...
        print(f"-> Generated Spearman heatmap: {config.correlation_heatmap_png_path(dataset_name).name}")
        ...
        strongest_correlations_path = (
            config.OUTPUT_DIR / f"strongest_correlations_top_features_{dataset_name}.csv"
        )
        strongest_correlations.to_csv(strongest_correlations_path, index=False)
        print(f"-> Saved strongest correlations: {strongest_correlations_path.name}\n")
